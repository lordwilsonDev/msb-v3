...
One hypothesis, one experiment at a time · failure conditions written **before** running · UNKNOWN never becomes GREEN · no expansion of MSB-v3 (§2/§25: no UI, SaaS, feature work, extra orchestration) · frozen artifacts are never edited — new versions only · any critical RED → stop, document, park, repair, retest · discipline scales to task size.
...
| **P5 Experiment (post D01–05 green)** | one H1 experiment, then attack, ablation, replication, formal check (2,880-config target), multi-model, independent review | — | Wilson greenlights 01–05 | per blueprint §24 | **Not scheduled.** No D06 until 01–05 are green (§27) |
...
1. **Self-grading:** every current number comes from the system's own tests, run by AI agents. Independent scrutiny is a §24 gate; nothing before it should be called "verified" in a paper.
...
6. **Scope drift:** the lane has already crossed into engineering once (§2 tension). Any new job must answer the seven questions in blueprint §2 or it does not enter the lane.
