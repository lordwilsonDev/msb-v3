...
def _parse_histogram(metric_lines: list[str]) -> dict[str, list[tuple[float, int]]]:
    ...
    for line in metric_lines:
        ...
        try:
            ...
            continue
        out[harness].append((edge, int(float(count_s))))  # "1.0" for ints
    for key in out:
...
def main() -> int:
    ...
    buckets = _parse_histogram([ln for ln in lines if "_bucket{" in ln and "latency" in ln])
    ...
    ready = [ln for ln in lines if ln.startswith("msb_v3_ready ")]
