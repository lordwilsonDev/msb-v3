...
| `msb_v3/mission` | OPTIONAL | Mission spine — versioned Mission object, frozen state machine, chain-evidenced store (Phase 1 of the agent control plane, `docs/blueprints/2026-09-25-agent-control-plane.md` §4). Off the HTTP/cockpit/worker path; the store is the spine later phases build on. |
...
| `msb_v3/vault` | OPTIONAL | Frontmatter parsing + vault-wide hygiene lint (missing required fields, invalid/inverted dates, duplicate ids, dangling `[[wikilinks]]`) — read-only, exposed via the `vault_lint` governed tool. Off canonical path. |
