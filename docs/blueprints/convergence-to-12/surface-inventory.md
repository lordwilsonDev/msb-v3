...
| S2 | `api/tenant_chat.py` — placeholder | `src/msb_v3/api/tenant_chat.py` | Gate test (`tests/api/test_tenant_chat_gate.py`) asserted it was NOT mounted; docstring dated 2026-08-15 | **RESOLVED (2026-09-11): CUT** | Forensic audit found no live plan referencing it — a pure echo handler, never mounted, no wiring anywhere. Deleted the module, its gate test, and the `PARKED_UNMOUNTED` entry in `test_no_dead_routers.py`. If tenant-scoped LLM routing is designed later, it starts fresh rather than resuming this stub. |
...
| S5 | Multimodal interfaces (`VisionClaw` / `HapticHeartbeat` / `SpeechFunctions`) | `src/msb_v3/triumvirate/multimodal_interfaces.py` | All methods return `status: "parked"` + `blocked_on: "mac-mini-storage"`; `/triumvirate/multimodal/*` routes 503 unless `MSB_MULTIMODAL_ENABLED=1`; both sides tested | **RESOLVED M3 (2026-08-17): PARK (flag-gated, code-relabeled)** | Dated decision: **not** implementing a narrow contract now — no core-path failure demands it (Rule 3), and `MSB_MULTIMODAL_ENABLED` defaults off with 503 fail-closed + both-side tests, so it is outside the default release path and honestly labeled. **2026-08-17 convergence pass:** the code now literally says `parked` (not `stub`) — each interface returns `{"status": "parked", "blocked_on": "mac-mini-storage"}` with a dated module docstring explaining the disk-space gate (the sovereign node was at 100% and the enrollment-era caches were the cause). Metric guards at the API call sites updated to skip `parked` payloads; tests updated to assert the parked status. The last live `stub` labels in the shipping surface are gone. |
...
## Convergence pass 2026-08-17 — audit of the remaining markers

Re-ran the marker sweep that produced the original 14 `stub` / 8 `NotImplementedError` / 32 `Phase 2` counts. Results:

| Marker | Count now | Verdict |
|---|---|---|
| `"stub"` in `src/` | 19 hits | **Mostly legitimate mode labels, not unfinished stubs.** `StubCharger` / `StubScanner` (deterministic offline flywheel default, `papers_scanned: 0`), `StubModel` (`MSB_CONVERSATION_MODEL=stub` deterministic fallback), `stub_mode` health checks, and the factory/conversation fixtures. The **3 real stub labels (multimodal) are now `parked`** (S5 above). Zero unlabeled shipping stubs remain. |
| `NotImplementedError` | 11 sites | **All abstract-interface contracts with concrete implementations** — `Expert.analyze` (overridden by `DomainExpert` + the LLM-backed panel), `VectorStore` (overridden by `QdrantVectorStore` / `SQLiteVectorStore`), `VectorIndex.search` (overridden by `Vector/Structural/TemporalIndex`). These are the interface seam, not stubs: **KEEP**, no action. |
| `Phase 2` refs | 47 hits | **Descriptive provenance comments, not deferred work.** Each references the completion-blueprint phase that *implemented* the feature (evidence spine, hybrid model router, context builder, flywheel, governed loop). The one that could have been real deferred work — guard.py's "record_action (Phase 2 loop wires this)" — was verified: `check_run` is called before every flywheel stage (`engine.py:208`) and `record_action` is exercised by `test_record_action_appends`. **KEEP**, no action. |

**New P3 coverage added this pass:** `test_kill_switch_armed_mid_run_halts_loop` — arms the kill switch AFTER a turn starts and proves the loop halts at the next stage transition with an audited reason (not just that the guard returns a verdict).
