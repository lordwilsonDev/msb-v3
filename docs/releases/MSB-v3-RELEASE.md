...
| Ledger extracted as a standalone library (P4): `msb_ledger` with zero `msb_v3` imports; `msb_v3.uac` is a thin alias shim, so all ~73 consumers + tests work unchanged | `tests/uac/test_ledger_extraction.py` (38 guards) |
...
| Governed-loop console (M8): a single-page browser console at `/console` to run a task, see the gate verdict + tool trace, replay a run from its events, and list past runs — a client of the existing operator-gated API (token entered once, kept in sessionStorage), no new backdoor. Shows live ActionGate verdict counters (SAFE/REVIEW/BLOCK/FAIL) + p50/p95 latency from the public `/metrics/prometheus` scrape (parsed in-page, no bearer header). Live-verified end-to-end in Chromium: token gate → PASS run → replay of a DENIED run → metrics strip rendered | `src/msb_v3/api/console.py` + `tests/api/test_console.py` (12 tests) + `artifacts/console/` screenshots + README + `metrics-strip-verified.md` |
