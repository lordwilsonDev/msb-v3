...
| Field | Value | Source |
|---|---|---|
| Name / version | `msb-v3` `0.5.0` | `pyproject.toml` |
...
3. ✅ ~~One secret unset~~ — **fixed 2026-08-14**: `OPENAI_API_KEY` is now set (the `/v1` adapter is configured — see §5), so the adapter no longer fails closed with 503. `TAVILY_API_KEY` set (flywheel feed), `MSB_OPERATOR_TOKEN` set (control-surface auth). (The remote frontier seam + `DEEPSEEK_API_KEY` were retired 2026-09-09, D1 — the `/v1` adapter is the local surface only.)
